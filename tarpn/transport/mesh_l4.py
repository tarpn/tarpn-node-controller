...
class DatagramTransport(DTransport):
    ...
    def write_to(self, address: str, data: Any) -> None:
        can_route, mtu = self.network.route_packet(MeshAddress(self.port))
        ...
        if can_route:
            if len(data) <= mtu:
                ...
                self.network.send_datagram(dest, header, encoded_data)
            else:
...
class DatagramProtocol(L4Protocol, LoggingMixin):
    ...
    def bind(self, protocol_factory: Callable[[], DProtocol],
        ...
        transport = DatagramTransport(self.l3_protocol, port, local_address, None)
    ...
    def connect(self, protocol_factory: Callable[[], DProtocol],
        ...
        if port in self.connections:
            raise RuntimeError(f"Connection to {port} is already open")
        protocol = protocol_factory()
        transport = DatagramTransport(self.l3_protocol, port, local_address, remote_address)
        protocol.connection_made(transport)
        self.connections[port] = (transport, protocol)
        return protocol
